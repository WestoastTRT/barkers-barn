...
export async function submitBarnOrder(data: OrderInput): Promise<OrderResult> {
  const name = data.name.trim();
  const email = data.email.trim();
  if (name.length < 2) return { ok: false, error: "Name the order." };
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return { ok: false, error: "Need a real email so the barn can confirm." };
  }
  ...
  if (merch.length === 0) {
    return { ok: false, error: "No barn merch in the bag." };
  }
  ...
  const rows = await sql<{ id: number }>`
    ...
    returning id
  `;
  return { ok: true, id: rows[0]?.id ?? 0, total };
}
